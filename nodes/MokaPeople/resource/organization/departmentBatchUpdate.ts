...
/**
 * 修改组织部门接口
 * 接口路径：POST /v1/org/department/batchUpdate
 * 文档：Moka People API → HCM → 修改组织部门接口
 *
 ...
 *
 * 必填字段（按官方文档）：
 *   - nodeUid           部门 ID（修改场景必须）
 *   - nodeCode          部门编码
 *   - effectDate        生效日期
 *   - superiorDept      上级部门（修改场景下 superiorDeptCode / superiorDeptId 非必须，可任传其一）
 *
 * 响应：data 为数组，每个元素对应单条写入结果（含 nodeUid / nodeCode / code / msg）。
 */

const PATH = '/v1/org/department/batchUpdate';
...
const properties: INodeProperties[] = [
	{
		displayName:
			'修改组织部门接口。必填字段：nodeUid（部门 ID）、nodeCode（部门编码）、effectDate（生效日期）、' +
			'superiorDept（上级部门，其中 superiorDeptCode 与 superiorDeptId 任传其一即可）。' +
			'<br>详细字段含义与示例请参考：' +
	...
	},
	{
		...
		required: true,
		description:
			'在 Moka People 后台「设置」→「对外接口设置」中为「变更组织部门接口」配置的接口编码',
	},
	{
		...
		required: true,
		description:
			'只需填写数组 JSON（<code>[{...}, {...}]</code>），每个对象代表一条待修改的部门，代码会自动包装为 <code>{ "data": [...] }</code> 发送。<br>' +
			'必填字段：<code>nodeUid</code>、<code>nodeCode</code>、<code>effectDate</code>、<code>superiorDept</code>。' +
			'其它字段（<code>deptSampleName</code> / <code>deptType</code> / <code>deptHierarchy</code> / ' +
			'<code>deptDirector</code> / <code>deptHrbp</code> / <code>treeOrder</code> / <code>extendFields</code> 等）请参考' +
...
const operation: ResourceOperations = {
	name: '修改组织部门',
	value: 'departmentBatchUpdate',
	description: '修改组织部门 (POST /v1/org/department/batchUpdate)',
	action: '修改组织部门',
	order: 30,
	options: properties,
